...
%%%%%%%%%%%	3.2 
%tablir la fonction de transfert du moteur

zpk(Tmd,'v')			% Pour voir ou sont les poles ( 'v') est la car c est une output
b1=Tmd.num{1}(2); 			% coefficient numrateur de Tmd
a1=Tmd.den{1}(2);				% coefficient dnonominateur de Tmd
...
%%%%%%%4.1.1 
%tudier la stabilit du correcteur proportionnel en fonction de son gain K
...
%B1 gain statique du systme desir
%1.
B1=T1d.num{1}(2);
%2.
Alpha=exp(-Te/TauxPI);
...
Tmd_BOpi=series(Kpi,Tmd);	%Fct de transfert BO avec gain PI
...
T3d=tf(num4,den4,Te,'variable','z');
T4d=series(T2d,T3d);
...
[P2d]=T4d.den{1}; % Tableau contenant les coefficients du dénominateur
...
Tmd_BOpid=Kpid*Tmd;   % fonction de transfert en B0
Tmd_BFpid=feedback(Tmd_BOpid,1);

step(T1,T1d,Tmd_BFpi,T2,T2d,T3d,Tmd_BFpid)   % réponse à un échelon
legend('T1','T1d','Tmd_BFpi','T2','T2d','T3d','Tmd_BFpid')
%%%%%%%%4.3.3
